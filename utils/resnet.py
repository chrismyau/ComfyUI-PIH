...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            input_f, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False
        )
...
class ResNet_PIH(nn.Module):
    def __init__(
        ...
        super(ResNet_PIH, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
class MobileNetV3(nn.Module):
    def __init__(
        ...
        self.EffNet = mobilenetv3_small(input_f,num_classes)
        self.sigmoid = sigmoid
...
class PIHNet(nn.Module):
    def __init__(
        ...
        super(PIHNet, self).__init__()
        self.model_bg = ResNet_PIH(BasicBlock, [3, 4, 6, 3], input_f=4, **kwargs)
...
def resnet18(pretrained=False, input_f=4, num_classes=1000, **kwargs):
    ...
    """
    model = ResNet(
        BasicBlock, [2, 2, 2, 2], input_f=input_f, num_classes=num_classes, **kwargs
    )
    if pretrained:
...
def resnet18_m(pretrained=False, num_classes=1000, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(
        BasicBlock, [2, 2, 2, 2], input_f=1, num_classes=num_classes, **kwargs
    )
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls["resnet18"]))
    return model
